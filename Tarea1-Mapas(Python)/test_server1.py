...
from getJson_test import get_json_aviones, get_json_buques
import time
import os
import json
...
class MyHandler(BaseHTTPRequestHandler):
	...
	def do_GET(s):
		...
		if "py$" in s.path:  # Asumo que el path de request va a tener un py$, recordar pedir los datos con eso
			...
			centerx = round(float(valores['centerx']), 1)
			centery = round(float(valores['centery']), 1)

			zoom = int(valores['zoom'])
			...
			print("[STARTING FUNCTION] get_json_buques")
